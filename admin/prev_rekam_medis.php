...
<body>
	...
	<table class="table">
		...
		if (!isset($_POST['cari']) && empty($_POST['pencarian'])) {
			$sql = mysqli_query($kon, "SELECT * FROM tb_rekam_medis ORDER BY id desc ");
		...
		?>
			...
			$sql4	=	mysqli_query($kon, "SELECT * FROM tb_pasien WHERE nik_pasien='$_POST[pencarian]'");
			$bc4	=	mysqli_fetch_array($sql4);

			$sql = mysqli_query($kon, "SELECT * FROM tb_rekam_medis WHERE id_pasien='$bc4[id_pasien]' ORDER BY id desc ");
			while ($bc = mysqli_fetch_array($sql)) {
